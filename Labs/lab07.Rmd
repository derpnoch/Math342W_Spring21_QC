...
storms %>%
  ...
  mutate(start_date = paste(wday(start_date), paste(months(start_date), day(start_date), sep = " "), year(start_date), sep = ", "))
```
...
storms %>% 
  group_by(name) %>% 
  summarize(Distance = sum(dist_from_prev), Displacement = paste(round(last(lat) - first(lat), 2), round(last(long) - first(long), 2), sep = " / "))
