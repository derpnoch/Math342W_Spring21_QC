...
No, not exactly correct because the number is too small, we experienced numerical underflow.
...
Y[,1] = as.numeric(x_3_ord == "infraction")
Y[,2] = as.numeric(x_3_ord == "misdimeanor")
Y[,3] = as.numeric(x_3_ord == "felony")
...
The row sum of each row should be either a 0 or 1 due to a 100 rows.
...
The column sum, each columns should equal the total amount who committed a infraction, misdemeanor or felony. 
...
X[,6] = sample(rep(c(0,1), times = c(76,24), size = 100))
...
```{r}
punch_holes = matrix(nrow = n, ncol = c, sample(c(rep(1, n*c*0.70), rep(30, n*c*.30))))

for(nRow in 1:n){
  for(nCol in 1:c){
    if(punch_holes[nRow,nCol] == 30){
      R[nRow, nCol] = NA
    }
  }
}
...
```{r}
#The 5% ile
fifth = quantile(v, probs = 0.05)
fifth

#Theoretically
qnorm(.05, mean(v), sd(v))

#Yes because v is the random realization from mean that is -10 and sd = 10 
```
...
#Yes because v is the random realization we have set and comparing it between the two result. 
```
